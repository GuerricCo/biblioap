...
describe('Library Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 28830 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.libraries()[0]).toEqual(expect.objectContaining({ id: 28830 }));
  });

  describe('trackId', () => {
    it('should forward to libraryService', () => {
      const entity = { id: 28830 };
      vitest.spyOn(service, 'getLibraryIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.librariesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
