...
public interface ReservationRepository extends JpaRepository<Reservation, Long>, JpaSpecificationExecutor<Reservation> {
    ...
    @Query(
        value = "select reservation from Reservation reservation left join fetch reservation.library left join fetch reservation.book left join fetch reservation.member",
    ...
    @Query(
        "select reservation from Reservation reservation left join fetch reservation.library left join fetch reservation.book left join fetch reservation.member"
    )
    List<Reservation> findAllWithToOneRelationships();

    @Query(
        "select reservation from Reservation reservation left join fetch reservation.library left join fetch reservation.book left join fetch reservation.member where reservation.id =:id"
    )
